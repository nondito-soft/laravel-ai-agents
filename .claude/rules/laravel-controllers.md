...
- Render via the **active frontend rule** (`FRONTEND` config in AGENTS.md → `rules/frontend/{stack}.md`). The response **must always include `breadcrumbs` and `pageTitle`**, whatever the stack. The data array is identical across stacks — only the render call differs:
