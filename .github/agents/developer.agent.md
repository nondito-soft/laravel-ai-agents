...
- **Service** — all business logic lives here. Extend `BaseModelService` for new resources. Wrap multi-step ops in `DB::transaction()`. Return model or `false`, not exceptions. Log activity manually (clone → mutate → `logActivity()`). Every public method gets a `/** ... */` block.
