...
- `.github/instructions/laravel-services.instructions.md` — service-layer conventions
...
- `.github/instructions/laravel-tests.instructions.md` — test conventions
- `.github/instructions/frontend/{FRONTEND}.instructions.md` — **only the file matching the active `FRONTEND` flag** (vue-inertia / react-inertia / livewire / blade)
...
- **Database** — migration following the migrations instructions. New tables get `is_active`, `created_by`, `updated_by`, `timestamps()`, `softDeletes()`. Altering a table → a new `*_add_*_to_*_table` migration, never edit a shipped migration.
- **Model** — `$fillable` (never `$guarded = []`), `$casts`, relationships, `SoftDeletes` where appropriate.
- **Service** — all business logic lives here. Extend `BaseModelService` for model-backed resources; an orchestration/helper service with no table does NOT extend it (and `use`s `App\Concerns\LogsActivity` if it logs). Delegate standalone foreign-model queries/writes to that model's service rather than inlining `OtherModel::...`; reading through an Eloquent relationship you already hold is fine. Wrap multi-step ops in `DB::transaction()`. Return model or `false`, not exceptions. Log activity only via the `LogsActivity` trait — no inline `activity()`: `logActivity($model, $event, $message, $attributes, $old)` (message as string) for resource ops, `logOperation($message)` for subject-less system ops. Every public method gets a `/** ... */` block.
...
- **Frontend** — implement per the **active `FRONTEND` stack only**, following `frontend/{FRONTEND}.instructions.md`. Wire to the controller's response data, reuse existing components/layouts, keep UI strings translated. Never hardcode another stack's API (no `Inertia::render` in a Blade project, etc.).
